...
double trilinear(double x, double y, double z)
{
	...
	int		X,Y,Z;	// volume width, length and depth (index space)
	short	*vol;
	double	v;
	...
	if((a>=0&&a<(X-1))&&(b>=0&&b<(Y-1))&&(c>=0&&c<(Z-1)))
	{
		v=	vol[ c   *Y*X+ b   *X+ a   ]*(1-x)*(1-y)*(1-z) +
			vol[ c   *Y*X+ b   *X+(a+1)]*x*(1-y)*(1-z) + 
			vol[ c   *Y*X+(b+1)*X+ a   ]*(1-x)*y*(1-z) + 
			vol[(c+1)*Y*X+ b   *X+ a   ]*(1-x)*(1-y)*z +
			vol[(c+1)*Y*X+ b   *X+(a+1)]*x*(1-y)*z + 
			vol[(c+1)*Y*X+(b+1)*X+ a   ]*(1-x)*y*z + 
			vol[ c   *Y*X+(b+1)*X+(a+1)]*x*y*(1-z) + 
			vol[(c+1)*Y*X+(b+1)*X+(a+1)]*x*y*z;
	}
	else
...
void addtovertices(VERTICES *vertices, VERTEX v)
{
	if(vertices->count == vertices->max)
	{
		...
		for(i=0;i<vertices->count;i++)
			new[i]=vertices->ptr[i];
		if(vertices->ptr != NULL)
			free((char *)vertices->ptr);
		vertices->ptr=new;
	}
...
void addtotriangles (TRIANGLES *triangles, TRIANGLE t)
{
	if (triangles->count == triangles->max)
	{
		...
		for(i=0;i<triangles->count;i++)
			new[i]=triangles->ptr[i];
		if(triangles->ptr!=NULL)
			free((char *)triangles->ptr);
		triangles->ptr=new;
	}
	triangles->ptr[triangles->count++]=t;
}

/* triangle: called by polygonize() for each triangle; write to stdout; store triangle */
int triangle(int i1, int i2, int i3, VERTICES vertices, TRIANGLES *triangles)
{
	addtotriangles(triangles,(TRIANGLE){i1,i2,i3});	
	return 1;
}
...
{
	double	f = trilinear(point->x, point->y, point->z);
	
	v->x=trilinear(point->x+p->delta,point->y,point->z)-f;
	v->y=trilinear(point->x,point->y+p->delta,point->z)-f;
	v->z=trilinear(point->x,point->y,point->z+p->delta)-f;
...
int dotet (CUBE *cube, int c1, int c2, int c3, int c4, PROCESS *p)
{
	...
	apos=(a->value>0.0);	if(apos)	index+=8;
	bpos=(b->value>0.0);	if(bpos)	index+=4;
	cpos=(c->value>0.0);	if(cpos)	index+=2;
	dpos=(d->value>0.0);	if(dpos)	index+=1;
	...
	switch (index)
	{
		case 1:	 return triangle(e5, e6, e3, p->vertices, &p->triangles);
		case 2:	 return triangle(e2, e6, e4, p->vertices, &p->triangles);
		case 3:	 return triangle(e3, e5, e4, p->vertices, &p->triangles) &&
						triangle(e3, e4, e2, p->vertices, &p->triangles);
		case 4:	 return triangle(e1, e4, e5, p->vertices, &p->triangles);
		case 5:	 return triangle(e3, e1, e4, p->vertices, &p->triangles) &&
						triangle(e3, e4, e6, p->vertices, &p->triangles);
		case 6:	 return triangle(e1, e2, e6, p->vertices, &p->triangles) &&
						triangle(e1, e6, e5, p->vertices, &p->triangles);
		case 7:	 return triangle(e1, e2, e3, p->vertices, &p->triangles);
		case 8:	 return triangle(e1, e3, e2, p->vertices, &p->triangles);
		case 9:	 return triangle(e1, e5, e6, p->vertices, &p->triangles) &&
						triangle(e1, e6, e2, p->vertices, &p->triangles);
		case 10: return triangle(e1, e3, e6, p->vertices, &p->triangles) &&
						triangle(e1, e6, e4, p->vertices, &p->triangles);
		case 11: return triangle(e1, e5, e4, p->vertices, &p->triangles);
		case 12: return triangle(e3, e2, e4, p->vertices, &p->triangles) &&
						triangle(e3, e4, e5, p->vertices, &p->triangles);
		case 13: return triangle(e6, e2, e4, p->vertices, &p->triangles);
		case 14: return triangle(e5, e3, e6, p->vertices, &p->triangles);
	}
	return 1;
}
...
int docube (CUBE *cube, PROCESS *p)
{
	...
	for(polys=cubetable[index];polys;polys=polys->next)
	{
		...
		for (edges = polys->list; edges; edges = edges->next)
		{
			...
			if (++count > 2 && ! triangle(a, b, c, p->vertices, &p->triangles)) return 0;
			if (count < 3) a = b;
			b = c;
		}
	}
...
CORNER *setcorner (PROCESS *p,int i,int j,int k)
{
	...
	l->j=j;
	l->k=k;
	l->value=c->value=trilinear(c->x,c->y,c->z);
...
TEST find (int sign, PROCESS *p, double x, double y, double z)
{
	...
	for(i=0;i<10000;i++)
	{
		...
		test.value=trilinear(test.p.x,test.p.y,test.p.z);
...
char *mycalloc6(int nitems,int nbytes)
{
	char	*ptr=calloc(nitems, nbytes);
...
char *mycalloc8(int nitems,int nbytes)
{
	char	*ptr=calloc(nitems, nbytes);
 ...
 *		int triproc (i1, i2, i3, vertices, triangles)
 *		int i1, i2, i3 (indices into the vertex array)
 *		VERTICES vertices (the vertex array, indexed from 0)
 *		TRIANGLES triangles (the triangle array, indexed from 0)
...
char *polygonize (PROCESS *p,VOLUME *vol,double size,int bounds,double x,double y,double z,int mode)
{
	...
	p->trash=(Trash*)calloc(1,sizeof(Trash));
	p->trash->ptr=0;
	p->trash->next=0;
	trash=p->trash;
	...
	p->triangles.count=p->triangles.max=0;	/* no triangles yet */
	p->triangles.ptr=NULL;
